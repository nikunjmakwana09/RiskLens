...
# ---------------------------------------------------
# Transform using saved preprocessing contract
# ---------------------------------------------------
    ...
    for i in range(num_samples):
        ...
        for _, row in contrib_df.iterrows():
            top_rows.append({
                ...
                "direction": "increase_diabetes_risk" if row["shap_value"] >= 0 else "decrease_diabetes_risk"
            })
...
def build_explainability_summary(
    ...
    summary = {
        ...
        "notes": {
            "interpretation": "Positive SHAP values push prediction toward diabetic class, while negative SHAP values push prediction toward non-diabetic class.",
            "clinical_warning": "These explanations support transparency but must not be used as standalone medical evidence."
        }
    }
...
def run_shap_pipeline() -> None:
    logger.info("Starting diabetes SHAP explanation pipeline...")
    ...
    logger.info("Diabetes SHAP explanation pipeline completed successfully.")
