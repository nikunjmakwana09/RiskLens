...
MUTUAL_INFORMATION_PLOT_PATH = os.path.join(REPORT_DIR, "mutual_information_top8.png")
...
REQUIRED_COLUMNS = NUMERICAL_FEATURES + [TARGET_COLUMN]
...
def load_data() -> pd.DataFrame:
    logger.info("Loading processed diabetes dataset...")
...
def validate_dataset(df: pd.DataFrame) -> Tuple[List[str], List[str]]:
    ...
    if len(numeric_features) != len(NUMERICAL_FEATURES):
        missing_numeric = sorted(list(set(NUMERICAL_FEATURES) - set(numeric_features)))
        raise ValueError(f"Expected diabetes numerical features missing: {missing_numeric}")
...
def plot_correlation_heatmap(df: pd.DataFrame, numeric_features: List[str]) -> pd.DataFrame:
    ...
    plt.figure(figsize=(12, 9), dpi=FIG_DPI)
...
def save_classwise_means(df: pd.DataFrame, numeric_features: List[str]) -> pd.DataFrame:
    ...
    grouped.columns = ["non_diabetic_mean", "diabetic_mean"]
    grouped["absolute_difference"] = (grouped["diabetic_mean"] - grouped["non_diabetic_mean"]).abs()
    grouped["relative_difference_percent"] = np.where(
        grouped["non_diabetic_mean"].abs() > 1e-12,
        (grouped["absolute_difference"] / grouped["non_diabetic_mean"].abs()) * 100,
        np.nan
    )
...
def build_eda_summary(
    ...
    non_diabetic_count = int(target_distribution.get("0", 0))
    diabetic_count = int(target_distribution.get("1", 0))
    prevalence = round((diabetic_count / total_rows) * 100, 4) if total_rows else 0.0
    ...
    recommendations = [
        ...
        "Consider robust scaling or outlier-tolerant models for high-outlier variables.",
        "Handle skewness during training-time preprocessing rather than altering the cleaned dataset.",
        "Prioritize recall, calibration, and threshold tuning because missed diabetic cases are clinically important.",
    ]
    ...
    return {
        ...
        "target_column": TARGET_COLUMN,
        "diabetes_prevalence_percent": prevalence,
        "class_balance": {
            "non_diabetic_count": non_diabetic_count,
            "diabetic_count": diabetic_count,
            "non_diabetic_rate_percent": round((non_diabetic_count / total_rows) * 100, 4) if total_rows else 0.0,
            "diabetic_rate_percent": round((diabetic_count / total_rows) * 100, 4) if total_rows else 0.0,
...
def build_eda_report_text(
    ...
    non_diabetic_rate = (target_distribution.get("0", 0) / total_rows) * 100 if total_rows else 0.0
    diabetic_rate = (target_distribution.get("1", 0) / total_rows) * 100 if total_rows else 0.0
    ...
    lines = [
        "DIABETES - EXPLORATORY DATA ANALYSIS INSIGHT REPORT",
        "=" * 80,
        "",
        "1. DATASET OVERVIEW",
        "-" * 80,
        ...
        "-" * 80,
        f"Non-Diabetic (0) Count    : {target_distribution.get('0', 0)}",
        f"Diabetic (1) Count        : {target_distribution.get('1', 0)}",
        f"Non-Diabetic (0) Rate     : {non_diabetic_rate:.2f}%",
        f"Diabetic (1) Rate         : {diabetic_rate:.2f}%",
        ...
        "-" * 80,
        "Glucose, BMI, insulin-related patterns, and related metabolic variables are typically central for diabetes discrimination.",
        "Missed diabetic cases can be clinically costly, so recall, calibration, and threshold tuning should be prioritized.",
        "Outliers and skewness should be handled during training-time preprocessing rather than changing the cleaned dataset.",
        "",
        ...
        "Consider robust preprocessing for skewed and outlier-prone variables.",
        "",
        "7. GOVERNANCE NOTE",
        "-" * 80,
...
def run_eda_pipeline() -> None:
    logger.info("Starting Diabetes EDA pipeline...")
