...
def normalize_importance(values: pd.Series) -> pd.Series:
    values = values.copy().fillna(0.0).clip(lower=0.0)
    total = values.sum()
    if total <= 0:
...
def validate_dataset(df: pd.DataFrame) -> None:
    ...
    unique_targets = sorted(df[TARGET_COLUMN].dropna().astype(int).unique().tolist())
...
def save_feature_importance_plot(
    ...
    plt.ylabel("Feature")
    plt.title(f"Top {min(top_n, len(plot_df))} Feature Importances - Heart Disease")
    plt.tight_layout()
...
def run_feature_importance_pipeline() -> None:
    logger.info("Starting heart disease feature importance pipeline...")
    ...
    payload = build_feature_importance_json(
        merged_df=merged_df,
        metadata=metadata
    )
    save_json(payload, FEATURE_IMPORTANCE_JSON_PATH)
    ...
    logger.info("Heart disease feature importance pipeline completed successfully.")
